<template>
  <div>
    <div>
      <div class="base-tit">
        <Breadcrumb item="设备管理,设备撤机"></Breadcrumb>
        <div class="head-right">
          <el-button type="text" @click="apply">申请撤机</el-button>
        </div>
      </div>
      <div class="searchBox">
        <p>
      ...
      <el-table :data="deviceDelData" border size="small" style="width:100%">
        <el-table-column
          v-for="(v, i) in tableData.slice(0,8)"
          :prop="v.tableProp"
          :label="v.tableLabel"
          :key="i"
        ></el-table-column>
    ...
    <device-del v-if="isApply" @close="close" @fresh="fresh"></device-del>
  </div>
</template>
<script>
import { post } from '../../../store/requestFacade.js'
import { mixins } from '@/core'
import { deviceDelColumn, parseDeviceDel } from '../own/deviceDel.js'
import { getAuditStatus, getActivateStatus } from '../own/status.js'
import deviceDel from './applyDel'
export default {
  ...
  methods: {
    ...
    handleRows(rows) {
      this.deviceDelData = parseDeviceDel([].concat(rows))
    }
  },
  mounted() {
    this.getModel()
    this.loadData()
    this.tableData = deviceDelColumn()
  }
}
</script>